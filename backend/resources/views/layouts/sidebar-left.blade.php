...
<aside class="navbar navbar-vertical navbar-expand-lg navbar-dark">
	<div class="container-fluid">
		...
		<h1 class="navbar-brand navbar-brand-autodark">
			<a
				href="{{ Auth()->user()->role === App\Enums\User\UserRole::Admin ? route('admin.dashboard') : route('hotel.dashboard',['hotel_id'=>Auth()->user()->id]) }}">
				<img style="min-height: 80px" src="{{ asset('/assets/images/light-logo.png') }}" alt="Tabler"
			...
			<ul class="navbar-nav pt-lg-3">
				@foreach ($menu as $item)
					@if (empty($item['sub']))
						{{-- Menu đơn --}}
						<li class="nav-item">
							<a href="{{ $item['routeName'] ? route($item['routeName'], isset($item['param']) && $item['param'] ? ['hotel_id' => Auth()->user()->id] : null) : '#' }}"
								class="nav-link {{ Route::currentRouteName() == $item['routeName'] ? 'active' : '' }}">
					...
					@else
						{{-- Menu có submenu --}}
						<li
							class="nav-item dropdown {{ in_array(Route::currentRouteName(), array_column($item['sub'], 'routeName')) ? 'active' : '' }}">
							<a href="#" class="nav-link dropdown-toggle" data-bs-toggle="dropdown" data-bs-auto-close="false"
							...
							<div
								class="dropdown-menu {{ collect($item['sub'])->contains(fn($sub) => Route::currentRouteName() == $sub['routeName']) ? 'show' : '' }}">
								<div class="dropdown-menu-columns">
									<div class="dropdown-menu-column">
										@foreach ($item['sub'] as $sub)
											<a href="{{ $sub['routeName'] ? route($sub['routeName'], isset($sub['param']) && $sub['param'] ? ['hotel_id' => Auth()->user()->id] : null) : '#' }}"
												class="dropdown-item {{ Route::currentRouteName() == $sub['routeName'] ? 'active' : '' }}">
												<span class="nav-link-icon d-md-none d-lg-inline-block">
													{!! $sub['icon'] !!}
												</span>
